...
public class TabuSearchForBestSolution {
	...
	public boolean tabuSearchII() {
		...
		if(tabuSearchForFiledSolution.tabuSearch()) {
			...
			int[][] tabuClient = new int[CLIENT_NUM][CLIENT_NUM];//客户禁忌表，用于存放变换的操作的关键客户
			int[][] tabuRoute = new int[CLIENT_NUM][incumbentSolution.getRoutesSolution().size()];//路径禁忌表，用于存放变换操作的关键客户和路径
